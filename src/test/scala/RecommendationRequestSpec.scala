...
class RecommendationRequestSpec extends FlatSpec with MockFactory with Matchers {
  ...
  it should "parse the response" in {
    ...
    val engine = stub[RecommendationEngine]
    (engine.sendQuery _).when(*).returning(engineResponse)

    new RecommendationRequest(engine).getRecommendations("userId") should be (List("52", "95"))
  }
}
